...
「登录页 · 浅色」使用 `blue-hour-air.glsl`，通过与浅色首页共用的 `blue-hour-palette.png` 呈现 AIR 椭圆色块与缓慢漂移动画，不使用首页的 PRISM2 板条。色块先混合色表坐标、柔化后再取色，避免深浅 RGB 直接叠加产生灰色交界；底部提亮也沿用同一色表。上部保留深靛色以承载白色欢迎语，下部为冰蓝与浅紫过渡；登录表单及排版保持不变。

登录页作为居中面板使用：1080×1080、32px 圆角、1.5px 内描边与柔和投影；背后主页模糊，面板保持清晰。「登录页 · 浅色 · 居中场景」以 1920×1080 展示面板的 80% 等比副本，背景为模糊主页快照，仅用于设计预览。

背景仅用于 Pen，速度设为 0 可暂停。布局、参数与来源见 [登录页 AIR 背景](web/design/asset/blue-hour-air.md)。
...
「仪表盘 · 浅色」为 1920×1925 设计稿。上方依次为筛选、用量概览、Token 堆叠柱状图，下方为模型请求日志；所有数值均为示例，尚未接入采集。

| 区域 | 约定 |
|---|---|
| 布局 | 保留 264px 侧栏；主区左右内边距 56px，内容宽 1544px，区块间距 28px |
| 表面 | 页面中性 1，内容区纯白，分隔与边框中性 6，表头中性 3；侧栏沿用首页浅色 |
| 图表 | 输入为主色 9，缓存读取为主色 8，输出为成功 9，缓存写入为警告 9 |
| 状态 | 彩色浅底配对应第 11 步文字；取消状态使用中性色 |
| 排版 | 中文思源黑体；数字、模型与供应商使用 Inter；标题 30px，指标 36px，表格正文 14px |
| 组件 | 沿用现有圆角变量：控件、标签与分页 `radius-control=6px`，分段选择器外框 `radius-row=8px`，卡片与表格 `radius-menu=12px`；不使用胶囊式控件 |

字段来源、费用与计时口径见 [仪表盘说明](doc/design/web/dashboard.md)。这些尺寸仅描述该画布，不补全全站间距令牌。
