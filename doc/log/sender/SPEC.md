...
状态：草案。更新：2026-09-19。数据字段与口径见 [../../schema.md](../../schema.md)，总体目标见 [../README.md](../README.md)。
...
一条记录失败就是整场会话丢失，因此重试价值高于事件级方案。落盘时机与上限见下方议题 M6。
...
用独立文件而非 `settings.json`，因为 pi 未向扩展暴露设置管理器，且自定义字段容易被 pi 的校验或迁移影响。位置本身见下方议题 M7。
...
| M6 | 缓存落盘时机与上限 | A 仅失败后写。B 先写再发，成功后删。上限按条数或字节 |
| M7 | 配置位置 | A 独立 `telemetry.json`。B `settings.json`。C `pi.cmd` 注入环境变量 |
