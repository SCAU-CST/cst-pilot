...
```
        ...
        │     └─ U 盘凭据
        │          device.json        设备标识（无秘密）
        │          auth.json          access token（2 小时）
        │          cst-refresh.json   refresh token（可选，7 天）
        └─────────────────────────────────────────────────────
```
...
| 编号 | 方向 | 内容 | 接口 / 通道 |
|---|---|---|---|
| ① | 手机 → 前端 | 打开授权页，展示设备名与作用域 | HTTPS /oauth/device |
...
|---|---|
| 队员拒绝授权 | agent 提示重新扫码 |
| access_token 过期 | 静默刷新；无 refresh 时重新扫码 |
| 设备被吊销、队员改密 | 立即失效，重新扫码授权 |
| 网络不可用 | 提示检查网络，无离线模式 |
...
| 阶段 | 内容 | 状态 |
|---|---|---|
| M1 | 设备授权流、TOTP、设备管理、会话与任务绑定、短期网关令牌、pi 扩展 | 本 spec 主体 |
| M2 | 修机日志、遥测上报、附件 | 概要 |
| M3 | 任务生命周期动作、委员只读 | 概要 |
...
| 假设 | 对应设计 |
|---|---|
| agent 运行在机主电脑上 | 该环境不可信，令牌按可泄露处理 |
| U 盘是队员随身设备 | 持久凭据只允许存 U 盘 |
| 机主电脑可能带恶意软件 | 不扩大 OA 权限，靠短有效期与吊销兜底 |
| 队员手机用于确认 | 扫码确认与 TOTP 都在手机完成 |
...
| 2 | agent | 请求设备码，终端显示二维码与 6 位数字码 |
| 3 | 队员 | 手机扫码打开 OA 授权页 |
| 4 | OA | 要求重新登录并校验 TOTP |
| 5 | OA | 显示设备名、作用域、有效期 |
| 6 | 队员 | 确认或拒绝 |
| 7 | agent | 轮询令牌端点，换取 access token |
| 8 | agent | 创建修机会话，绑定所选任务 |
| 9 | agent | 申请短期网关令牌，正常对话与诊断 |
| 10 | OA | 到期或吊销后失效 |

说明：任务在授权完成后选择。授权页不显示具体任务，只看设备与作用域。

### 记住 7 天（可选）

1. 队员在授权页勾选「记住这台 U 盘 7 天」，默认关闭。
2. OA 返回 refresh token，agent 明文写入 U 盘。
3. 每次刷新换新令牌，旧令牌立即作废。
4. 到期、吊销、改密任一发生即失效。
...
1. 长期凭据只允许存在于队员 U 盘。
2. 机主电脑不保存任何长期凭据。
3. access_token 校验时查设备状态，吊销即时生效。
...
3. 提交日志前队员确认。
4. 无单上门时，队员用小程序现场报修生成任务，再在 agent 内选择。
...
1. 新 U 盘首跑显示二维码，扫码并完成 TOTP 后模型可用。
2. 未授权状态下，agent 无法调用模型与 OA 接口。
3. 关闭「记住」时，U 盘内不存在任何长期凭据。
4. 任务列表正确，创建会话后用量与审计挂在所选任务上。
5. 改密后既有令牌立即失效。
6. 在个人中心吊销设备后，该设备令牌立即失效。
7. 用量计入队员个人额度，可在概览页看到。

## 里程碑

| 阶段 | 交付 |
|---|---|
| M1 | OA 设备授权流、TOTP、设备管理页、会话与任务绑定、网关令牌签发；pi 扩展；模型调用 |
| M2 | 修机日志草稿与提交；遥测合并上报 |
