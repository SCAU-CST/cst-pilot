...
| `testlog/` | 已执行的环境、结果、缺陷与未决项；按日期分文件，本机记录，不入库 |
...
| ID | 调用 | 预期与对照重点 |
|---|---|---|
| B32 | safe 档，两条命令 | 文件落在 `outbox\<当天>\`；文件名含递增序号、中文档名与标题；打开无 Markdown，文件头一行概括，条目为环境 / 说明 / 命令三段，命令独占一行；UTF-8 BOM + CRLF，记事本无乱码 |
| B33 | 同日再调用两次；随后用已有序号修正其中一份；跨天再调用一次 | 序号递增为 02、03 且不重复；沿用序号重写后文件名随新标题变、序号不变、文件数不增；跨天从 01 重新开始 |
| B34 | command 含换行、title 为空、items 为空 | 整次调用失败并说明原因，不产生半成品文件 |
| B35 | 介质只读或 outbox 不可写 | 返回具体失败原因，不静默；对照 outbox 与机主系统无其他新增文件 |
