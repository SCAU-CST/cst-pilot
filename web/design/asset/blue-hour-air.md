...
`cst-pilot-web.pen` 的「登录页 · 浅色」使用 `blue-hour-air.glsl`。只用于 Pen 设计稿，不代表网页端已实现。
...
在椭圆半径的 22% 内保持完整权重，22%–72% 之间逐渐衰减。按图层顺序混合五个色标位置（0.10 / 0.30 / 0.50 / 0.70 / 0.90），对这个数值场做九点柔化，最后查询与首页相同的 `blue-hour-palette.png`；色表在 Oklab 空间插值生成。
...
| `u_palette` | `asset/blue-hour-palette.png` | 与浅色首页共用五色色表 |
...
| 场景预览 | 「登录页 · 浅色 · 居中场景」，1920×1080 |
| 预览面板 | 按 80% 等比展示为 864×864，x=528、y=108 |
| 背后主页 | 高斯模糊 20px，叠加 `#060B0E26` 遮罩 |

场景预览使用 `login-preview/HRlE2.png`，来源于浅色主页的静态导出；登录面板仍使用动态 Shader。该快照只用于设计场景，正式网页应模糊真实主页，不加载此图片。预览面板为独立副本，不自动同步源组件。
